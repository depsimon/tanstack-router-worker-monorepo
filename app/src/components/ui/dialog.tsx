...
const DialogDescription = ({
	...
	<p
		...
		className={twMerge(
			"text-pretty text-base/6 text-mute group-disabled:opacity-50 sm:text-sm/6",
			className,
		)}
		ref={ref}
		{...props}
...
const DialogClose = ({ ...props }: ButtonProps) => {
	return <Button slot="close" intent="plain" {...props} />;
};
...
const DialogCloseIcon = ({
	...
	return props.isDismissable ? (
		<PrimitiveButton
			...
			slot="close"
			className={cx(
				"close absolute top-1 right-1 z-50 grid size-8 place-content-center rounded-xl hover:bg-accent-soft focus:bg-accent-soft focus:outline-hidden focus-visible:ring-1 focus-visible:ring-default sm:top-2 sm:right-2 sm:size-7 sm:rounded-md",
				className,
			)}
		>
